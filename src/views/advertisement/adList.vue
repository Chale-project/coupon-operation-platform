<template>
  <div style="background:#fff ;padding:25px;position: relative">
    ...
    <a-table size="middle" :columns="columns" :dataSource="loadDatas" :loading="loading" :pagination="pagination" :rowSelection="{type:'checkbox', selectedRowKeys: selectedRowKeys, onChange: updateSelect }">
      ...
      <template slot="Action" slot-scope="text,record,index">
        <span>
          ...
          <template v-if="record.homeShow=='no'">
            <a-divider type="vertical" />
            <a href="javascript:;" @click="modifyAd('homeAd', record, index)">设为首页广告</a>
          </template>
        </span>
      </template>
    </a-table>
    ...
    <a-modal :width="editType=='add'?'75%':editType=='edit'?'75%':editType=='detail'?'75%':'400px'" :title="modalTitle" :visible="visible" @ok="handleOk" :confirmLoading="confirmLoading" @cancel="handleCancel">
      ...
      <p v-else-if="editType=='homeAd'">确认设置所选为首页轮播广告吗？</p>
      <div class="modal-container" v-else>
        <a-form>
          <a-row :gutter="24">
            ...
            <a-col :md="12" :sm="24" v-show="editType!='add'">
              <a-form-item label="是否启用">
                <a-select v-model="forms.state" placeholder="请选择" :disabled="editType=='detail'">
                  <a-select-option value="enabled">启用</a-select-option>
                  <a-select-option value="disabled">禁用</a-select-option>
                </a-select>
              </a-form-item>
            ...
            <a-col :md="24" :sm="24">
              <a-form-item label="图片上传" :required="true">
                <a-upload :action="action" :headers="headers" :fileList="fileList" :showUploadList="false" :beforeUpload="beforeUpload" :remove="handleRemove" @change="handleChange">
            ...
            <a-col :md="24" :sm="24">
              <a-table size="small" :columns="filesColumns" :dataSource="filesLoadDatas" :loading="loading" :pagination="pagination" :locale="{emptyText: '没有上传的文件'}">
                ...
                <template slot="sort" slot-scope="text, record, index">
                  <a-input-number :min="1" v-model="record.sort" />
                </template>

                <template slot="picName" slot-scope="record">
                ...
                <template slot="outerLink" slot-scope="text, record, index">
                  <a-input v-model="record.outerLink" placeholder="请输入图片链接" />
                </template>
...
import { addAd, getAdInfo, modifyAd, removeAd, enabledAd, disabledAd, setHomeAd, getAdList } from '@/api/common'
...
const filesColumns = [
  ...
  },
  {
    ...
    width: '30%',
    dataIndex: 'outerLink',
    scopedSlots: { customRender: 'outerLink' }
  },
  {
    title: '缩略图',
    width: 60,
...
export default {
  ...
  methods: {
    ...
    modifyAd(e, o, index) {
      ...
      } else if (this.editType == 'homeAd') {
        this.modalTitle = '设为首页轮播广告'
        this.visible = !0
    ...
    handleChange({ file, fileList }) {
      ...
      if (file.status == 'done') {
        ...
        let _addFilesLoadDatas = this.addFilesLoadDatas

        _Arrfiles.forEach((item, index) => {
          const obj = {}
          if (_type == 'add') {
            obj.picPath = item.url
            obj.picName = item.name
            obj.outerLink = null
            obj.sort = index
          } else if (_type == 'edit') {
            obj.picPath = item.url
